...
function drawAvatar1() {
  ...
  for (let i = 0; i < stars.length; i++) {
    let star = stars[i];
    let scale = (sin(frameCount * star.speed + star.offset) + 1) / 2;
    let starSize = map(scale, 0, 1, star.size * 0.5, star.size * 1.5);
...
function drawAvatar2() {
  ...
  for (let i = 0; i < stars.length; i++) {
    let star = stars[i];
    let scale = (sin(frameCount * star.speed + star.offset) + 15) / 2;
    let starSize = map(scale, 0, 1, star.size * 0.5, star.size * 2.5);
    ellipse(star.x, star.y, starSize, starSize);
  }
 ...
 rotationAmount = sin(frameCount * .17) * .02;
...
function mousePressed() {
  ...
  if (currentState === "avatar1" && bruceHitboxAvatar1(mouseX, mouseY)) {
    currentState = "avatar2";
    crickets.stop(); 
     songs[selectedButton].loop();
  } else if (currentState === "avatar2" && bruceHitboxAvatar2(mouseX, mouseY)) {
    currentState = "avatar1";
